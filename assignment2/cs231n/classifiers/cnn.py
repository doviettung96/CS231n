...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        caches = []
        h, conv_cache = conv_forward_naive(X, W1, b1, conv_param)
        h, relu_cache = relu_forward(h)
        h, pool_cache = max_pool_forward_naive(h, pool_param)
        ...
        dh, dW3, db3 = affine_backward(dscores, caches.pop())
        dh, dW2, db2 = affine_relu_backward(dh, caches.pop())
        conv_cache, relu_cache, pool_cache = caches.pop()
        dh = max_pool_backward_naive(dh, pool_cache)
        dh = relu_backward(dh, relu_cache)
        dh, dW1, db1 = conv_backward_naive(dh, conv_cache)
